...
function convertDate(val, withWeek) {
    ...
    var hour = val.toString().substr(8, 2);
...
kLine.prototype = {
    ...
    paintItems: function () {
        ...
        yAxisOptions.region = yAxisOptions.region || { x: (high.toString().length * 7.4) - region.x, y: 0 - 3, height: region.height, width: region.x - 3 };  //* 左侧的文字{价格}
        //画y轴
        ...
        xAxisOptions.region = { x: 0, y: region.height + 2, width: region.width, height: 20 };  //* 底下的文字{日期}
...
function drawKL(ranges) {
    var kOptions = {
        ...
        region: { x: 0, y: 0, width: initialWidth , height: 160 }, // 主图标区域
        //region: { x: 45.5, y: 20.5, width: initialWidth - 45.5 - 20.5, height: 250 }, // 主图标区域

        barWidth: 5, spaceWidth: 2, horizontalLineCount: 3, verticalLineCount: 2, lineStyle: 'dashed', borderColor: 'gray', splitLineColor: '#eeeeee', lineWidth: 1,
        MAs: [
            { color: '#FDADC4', daysCount: 5 },
            { color: '#22C1DB', daysCount: 10 },
            { color: '#EE3073', daysCount: 20 },
            { color: '#8FBD13', daysCount: 60 }
            ],
        yAxis: {
        ...
        volume: {
            region: { x: 0, y: 180.5, height: 40, width: initialWidth },
        ...
        priceLine: {
            ...
            verticalLineCount: 7,
            horizontalLineCount: 1, lineStyle: 'dashed', borderColor: 'gray', splitLineColor: '#eeeeee',fillColor:'lightgray',alpha:.5,
            yAxis: {
                font: '11px Arial', // region: { },
                color: 'black',
                align: 'right',
    ...
    if(!painter){            
        ...
        painter = 1;
        getKLData(kType, function(data){
            painter = new Painter('canvasKL', kl, data);
            painter.paint();
        });
        // var data = getKLData(function(data){
        //     debugger;
        // });
        // painter = new Painter('canvasKL', kl, data);
        // painter.paint();
    }else if( painter && painter != 1 ){
